"""
Magbot Simulation + MSD without Walls (Rigid-Body Version)
----------------------------------------------------------
...
class SimParams:
    ...
    save_csv: bool = True
    save_plot: bool = True
    video_path: str = "magbot_sim_no_walls.mp4"
    gif_path: str = "magbot_sim_no_walls.gif"
    csv_path: str = "msd_no_walls.csv"
    plot_path: str = "msd_no_walls.png"
    seed: int = 7


# ---------------------------- ABP utilities ----------------------------

def simulate_abp_free(n_steps, dt, v0, Dr, Dt, rng):
    """Simulate ABP COM motion in open space (no reflection)."""
...
def animate_magbot(params: SimParams):
    """Animate a single rigid-body Magbot (no walls)."""
...
def run_msd_free(params: SimParams):
    rng = np.random.default_rng(params.seed + 2)
    pos = simulate_abp_free(
    ...
    if params.save_csv:
        pd.DataFrame({"tau": tau, "TAMSD_free": msd}).to_csv(params.csv_path, index=False)
        print(f"[Saved MSD data] {params.csv_path}")

    plt.figure(figsize=(7, 5))
    plt.plot(tau, msd, label="TAMSD (no walls)", color='k')
    plt.xlabel("Lag time τ")
    plt.ylabel("MSD(τ)")
    plt.title("MSD in Open Space (Rigid Magbot ABP)")
